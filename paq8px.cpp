...
#define PROGVERSION  "-t2-fix1"  //update version here before publishing your changes
...
static void printHelp() {
  printf("\n"
         ...
         "\n"
         "    Example:\n"
         "      " PROGNAME " -10 astro0_2048x1489x5x32f\n"
         "\n"
         "\n"
         "    -LEVEL:\n"
         "\n"
         ...
         "\n"
         "      -1 -2 -3 = compress using less memory (13, 23, 44 MB)\n"
         "      -4 -5 -6 -7 -8 -9 = use more memory (86, 170, 338, 674, 1346, 2690 MB)\n"
         "      -10  -11  -12     = use even more memory (5378, 10754, 21506 MB)\n"
         "\n"
         "\n"
         "    INPUTSPEC:\n"
         "\n"
         "    The input may be a FILE or a PATH/FILE.\n"
         "    The input file name must be properly formatted - it must contain image\n"
         "    width and number format information. For exact spec. see the GDCC 2021\n" 
         "    guidelines.\n"
         "\n"
         ...
         "\n"
         "    Example:\n"
         "      " PROGNAME " -8 multispec0_512x512x31x16u outputfolder/ -v -simd sse2\n"
         "    is equivalent to:\n"
         "      " PROGNAME " -v -simd sse2 multispec0_512x512x31x16u outputfolder/ -8\n");
}
  ...
  try {
    ...
    if( input.strsize() == 0 ) {
      printf("\nAn %s is required %s.\n", whattodo == DoCompress ? "input file" : "archive filename",
             whattodo == DoCompress ? "for compressing" : whattodo == DoExtract ? "for decompressing" : "");
      quit();
    }
    ...
    if (whattodo == DoCompress) {
      //parse filename - such as "astro1_2048x1489x5x32f"
      int width; //used
      int height; //ignored
      int channel; //ignored
      int format; //used
      int parsedItems = sscanf(input.c_str(), "%*[^_]_%dx%dx%dx%d%*c", &width, &height, &channel, &format);
      if (parsedItems != 4 || width <= 0 || (format != 16 && format != 32)) {
        quit("Parsing of filename failed.\n");
      }
      shared.fileFormat = format >> 3; //2 or 4
      shared.width = width;
      archive.putChar(shared.fileFormat);
      archive.putVLI(width);
    }
    else {
      shared.fileFormat = archive.getchar();
      shared.width = archive.getVLI();
    }

    if (shared.fileFormat == 2) {
      printf("\nSelected file format: int16/uint16\n");
    }
    else if (shared.fileFormat == 4) {
      printf("\nSelected file format: float32\n");
    }
    else {
    ...
    if (shared.width > 0) {
      ...
      quit("Error: illegal image width\n");
    }
    ...
    if( mode == COMPRESS ) {
      ...
      if( whattodo == DoExtract) {
        ...
        en.prefetch();
        decompressFile(&shared, fName, en, fSize);
      }
    }
